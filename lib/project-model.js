...
// Protobuf follows the same precedent: `buf.yaml` is optional, and a directory of
// *.proto IS the schema. Binding by extension means a proto tree with no buf.yaml is
// still checked — which is exactly the tree most likely to have never been checked.
function hasProtoSource(entries) {
  return entries.some((e) => e.isFile() && e.name.endsWith('.proto'));
}

// A hand-authored API spec. Named files only: a repo is full of .yaml and .json that
// are not API specs (CI workflows, manifests, fixtures, lockfiles), and handing them
// to spectral produces a torrent of findings about documents nobody meant as OpenAPI.
...
function hasApiSpec(entries) {
  return entries.some((e) => e.isFile() && SPEC_FILENAMES.has(e.name));
}
...
// A standalone mypy config. A section inside a shared file counts too — see below.
const MYPY_CONFIG_FILES = new Set(['mypy.ini', '.mypy.ini']);

/**
 * Has this directory opted IN to type checking?
 *
 * `contract` binds to every Python app unconditionally because it self-limits: an
 * app with no ORM models says so and skips, costing one line of output. mypy does
 * not self-limit. Pointed at an untyped codebase it reports thousands of errors,
 * every one of them true and none of them asked for — so binding it by default
 * would hand every Python repo in the world a wall of findings for a tool it never
 * chose, which is precisely the adoption cliff that teaches people to stop reading
 * the output.
 *
 * A mypy config is the decision. Nobody writes `[tool.mypy]` by accident.
 */
function hasMypyConfig(absDir, entries) {
  for (const e of entries) {
    if (e.isFile() && MYPY_CONFIG_FILES.has(e.name)) return true;
  }
  // Read for the section header rather than parsing: a TOML/INI parser earned for
  // one string would be a dependency, and these headers are unambiguous.
  for (const [file, header] of [
    ['pyproject.toml', '[tool.mypy]'],
    ['setup.cfg', '[mypy]'],
  ]) {
    try {
      if (fs.readFileSync(path.join(absDir, file), 'utf8').includes(header)) return true;
    } catch {
      // Absent or unreadable — not a signal either way.
    }
  }
  return false;
}
  ...
  function walk(absDir, relDir, depth) {
    ...
    for (const entry of entries) {
      ...
      if (entry.name === 'package.json') {
        ...
        if (looksLikeJsApp(absDir, entries)) {
          linters.add('eslint');
          // `tsc` alongside ESLint, never instead of it — different questions,
          // independently baselined.
          //
          // Bound on the TSCONFIG, not on package.json, and that distinction is
          // load-bearing. Every other adapter here holds an invariant the engine
          // quietly depends on: if a linter is BOUND, its code is PRESENT. Bind
          // tsc to every JS app and a plain-JavaScript package breaks it — the
          // adapter correctly reports `no-code`, nothing is written to the
          // manifest, and `detectDrift` then reports `new-linter: tsc` on that app
          // forever, because a bound linter with no manifest record is exactly how
          // a genuinely new linter looks.
          //
          // A tsconfig is also the better signal on its own terms: nobody has one
          // by accident, so it needs no config key to opt in.
      ...
      // mypy binds on an explicit signal only — see hasMypyConfig for why this one
      // is opt-in while `contract` above is not.
      if (linter === 'ruff' && hasMypyConfig(absDir, entries)) linters.add('mypy');
    }
    ...
    // Protobuf: lint AND breaking-change detection, always together. Shipping the
    // linter without the breaking check would guard the style of a schema while
    // leaving its COMPATIBILITY — the only thing a client of it actually depends on —
    // completely unwatched.
    if (hasProtoSource(entries)) {
      linters.add('buf');
      linters.add('buf-breaking');
    }

    if (hasApiSpec(entries)) linters.add('spectral');
    if (linters.size > 0) {
...
function discoverApps(projectRoot, opts = {}) {
  ...
  // Generated client types are opt-in TWICE OVER: a generator AND an output path. A
  // lockfile alone is not consent, and a JS app alone is not consent. A repo that types
  // its frontend by hand has usually made that choice on purpose, and a linter that nags
  // people about deliberate choices is a linter they uninstall.
  const codegenConfigured = Boolean(
    cfg.contract &&
      Array.isArray(cfg.contract.codegen) &&
      cfg.contract.codegen.some((t) => t && t.output)
  );

  const apps = [];
  for (const [dir, linterSet] of appLinters) {
    if (apiContractConfigured && linterSet.has('contract')) linterSet.add('openapi');
    if (codegenConfigured && linterSet.has('contract')) linterSet.add('codegen-drift');
