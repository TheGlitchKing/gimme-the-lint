...
// Statuses that mean "we did not look", as opposed to "we looked and it is fine".
// Exported so the JSON report and the engine cannot drift into disagreeing about
// which outcomes count as checked.
const UNCHECKED_STATUSES = new Set(['skipped', 'error', 'needs-baseline']);
...
async function runCheck(projectRoot, opts = {}) {
  ...
  // Units that did not actually CHECK anything. All three look like zero violations
  // and none of them are a clean bill of health (#19):
  //
  //   skipped         the linter or the app could not be reached
  //   error           the adapter itself blew up
  //   needs-baseline  there was nothing to diff against
  //
  // `error` is on this list for the same reason as `skipped`, and it is worth saying
  // out loud: an errored unit returns early with no `diff`, so it contributes 0 to
  // newViolations and the run goes green. The issue reported the skip half; the error
  // half is the same bug wearing a different status.
  const unchecked = results
    .filter((r) => UNCHECKED_STATUSES.has(r.status))
    .map((r) => ({ unit: r.unit, linter: r.linter, status: r.status, reason: r.reason || null }));

  const ok =
    newViolations === 0 &&
    !(opts.noStaleBaseline && staleEntries > 0) &&
    // Opt-in, deliberately. Defaulting this on would turn somebody's green CI red on a
    // minor upgrade, for a condition that was already true yesterday — the same
    // reasoning that kept --no-stale-baseline opt-in. The GH Action defaults it on,
    // because there the contract is ours to set.
    !(opts.failOnSkip && unchecked.length > 0);
  ...
  return {
    ...
    staleEntries,
    staleUnevaluated,
    // Always reported, flag or no flag. The flag decides whether it BLOCKS; it never
    // decides whether the reader is told.
    unchecked,
    failOnSkip: Boolean(opts.failOnSkip),
