 ...
 * @param {string} [props.fingerprintKey] A stable, file-independent name for the
 *   thing being complained about (e.g. "Deal.operating_expenses:writable"). When
 *   supplied it REPLACES file+message in the violation's identity, so the baseline
 *   survives the declaration being moved to another file or the message being
 *   reworded. Adapters that cannot name their subject that precisely omit it and
 *   get the default file+rule+message identity. See fingerprint.js.
 * @param {boolean} [props.neverBaseline] This violation means the code is broken
 *   RIGHT NOW — not "not yet ideal" — so it may never be silently grandfathered.
 *   Set from the RULE's own metadata by the adapter that produced it; the engine
 *   holds no hardcoded list, because rules belong to the linter that defines them.
 *   (Precedent: gitleaks findings are never baselined — a leaked secret is not
 *   technical debt.) It can still be EXCEPTED in config, which costs a human a
 *   written reason. See lib/baseline.js and lib/diff-engine.js.
...
function createViolation({
  ...
  return {
    ...
    fingerprintKey: fingerprintKey ? String(fingerprintKey) : null,
    neverBaseline: neverBaseline === true,
  };
}
