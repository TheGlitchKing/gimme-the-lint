...
class LinterAdapter {
  ...
  /**
   * Does this adapter check the WHOLE program regardless of the targets it is
   * handed, or only the files it is given?
   *
   * Almost every linter answers per-file: hand it two staged files and it reports on
   * those two. A type checker cannot — the type of an expression here depends on
   * declarations there — so `tsc` and `mypy` ignore `targets` entirely.
   *
   * The engine needs to know which it is dealing with for one reason: a baselined
   * violation that does not appear in the current run has either been FIXED or was
   * NEVER LOOKED AT, and those are the same absence. Only an adapter that saw
   * everything can tell them apart (see runCheck).
   */
  get wholeProgram() {
    return false;
  }
