  ...
  parse(stdout, stderr, code) {
    ...
    return (report.violations || []).map((v) =>
      createViolation({
        ...
        // Trust what the PROVIDER declares. This used to be inferred here, by exclusion
        // ("everything except lockfile-missing is a defect") — which worked for exactly
        // as long as there were two rules, and silently mis-classified the next two the
        // moment they were added. `route-without-response-model` fires 65 times on a real
        // codebase; making it un-baselineable by accident would have meant fixing 65
        // routes before your next commit, and nobody does that. They uninstall.
        //
        // Rules belong to the linter that defines them. The engine does not get a vote.
