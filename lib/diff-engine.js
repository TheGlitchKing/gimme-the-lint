...
function diff(current, baseline) {
  ...
  for (const [fp, violations] of byFingerprint) {
    // GATE 2 of 3. A violation the linter marks `neverBaseline` always counts as
    // new, no matter what the baseline file says.
    //
    // baseline-store excludes these at capture time, so a baseline written by this
    // version cannot contain one. This gate exists for the baselines that were NOT
    // written by this version: a file hand-edited to make the noise stop, or one
    // captured before the rule was classified as a defect. Trusting the file would
    // make the guarantee only as strong as the least careful edit anyone ever made
    // to it — and the whole point is that it does not depend on care.
    const allowed = violations[0] && violations[0].neverBaseline
      ? 0
      : Number(baselineCounts.get(fp)) || 0;
