...
async function baselineUnit(projectRoot, unit, opts = {}) {
  ...
  for (const linterId of unit.linters) {
    ...
    // GATE 3 of 3. Violations that were NOT captured, because they mean the code is
    // broken right now rather than merely imperfect.
    //
    // Reporting them here is not a nicety. Someone who runs `baseline` reasonably
    // believes they have just cleared the decks — and if these were silently
    // omitted, their very next push would be blocked by violations they thought
    // they had just baselined, with no explanation for why the command they were
    // told to run did not work. That user concludes the tool is broken, and they are
    // not being unreasonable. Say it now, at the moment they can still act on it.
    const refused = baselineStore.unbaselineable(violations);
...
/** Every violation a baseline run REFUSED to capture, across all units. */
function collectUnbaselineable(unitResults) {
  const refused = [];
  for (const u of unitResults || []) {
    for (const s of u.sections || []) {
      for (const v of s.unbaselineable || []) {
        refused.push({ app: u.appPath, linter: s.linter, violation: v });
      }
    }
  }
  return refused;
 ...
 * @returns {{unitCount, units, incomplete, unbaselineable}}
 */
async function runBaseline(projectRoot, opts = {}) {
  ...
  return {
    ...
    // Violations that were deliberately NOT grandfathered. These WILL block the
    // next push, and the user needs to hear that from the command they just ran —
    // not from the failure.
    unbaselineable: collectUnbaselineable(results),
  };
}
